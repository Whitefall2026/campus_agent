...
- 关键列：
  ...
  - `real_sender_id`：⚠️ 微信 4.x 实测该数字不总是 `2`（有的账号里“自己”
    是 `3` 等其它常量），也不是 `SenderName2Id` 的 rowid；群聊真实发送者
    wxid 写在正文前缀 `wxid_xxx:\n` 里。统一走
    `WeChatDB._resolve_sender_username()`：群聊取正文前缀、单聊按
    “自己/会话对方”判定（`_detect_self_sender_id()` 用文件传输助手等
    数据驱动识别“自己 id”），`SenderName2Id` 仅作老版本兜底；
  - `server_id`：服务端消息 id；
...
> 发表朋友圈已舍弃（4.x 自绘界面不可靠）；点赞/评论通过热激活 `mmui` UIA 树后，
> 基于 UIA 控件完成（`WeChat.Moment` / `SwitchToMoments`，见 `demo_moments_interact.py`）。
...
5. **发朋友圈已舍弃**；点赞/评论需热激活 `mmui` UIA 树（`WeChat.Moment`），
   树不可用或锁屏时无法操作；
