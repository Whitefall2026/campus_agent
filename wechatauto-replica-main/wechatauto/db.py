...
# 群聊消息正文自带发送者前缀，形如：
#   "wxid_nlc9x5u91kvp22:\n有氧仙人"
# 这是微信 4.x 消息库里最权威的发送者标识（real_sender_id 的数字不可靠，
# 实测既不是 SenderName2Id 的 rowid，也不是 contact.name2id 的 rowid）。
_MSG_SENDER_PREFIX_RE = re.compile(r"^(wxid_[0-9a-zA-Z_]+):\s*\n")
...
class WeChatDB:
    ...
    def get_messages(self, user: str, limit: int = 20, offset: int = 0) -> List[dict]:
        ...
        return self._run_msg_query(
            user,
            lambda conn, table: [
                self._msg_row_to_dict(r, chat=user) for r in conn.execute(
    ...
    def get_message_row(self, user: str, local_id: int) -> Optional[dict]:
        ...
        if isinstance(content, bytes):
            try:
                content = content[:200].decode("utf-8", errors="ignore")
            except Exception:
                content = ""
        sender_username = self._resolve_sender_username(
            sender_id, chat=user, content=content
        )
        return {
    ...
    def get_new_messages(self, user: str, since_seq: int = 0, limit: int = 200) -> List[dict]:
        ...
        return [self._msg_row_to_dict(r, chat=user) for r in rows] if rows else []

    def _msg_row_to_dict(self, r, chat: str = None) -> dict:
        ...
        sender_username = self._resolve_sender_username(
            sender_id, chat=chat, content=content
        )
        return {
            "local_id": r["local_id"],
            "type": mtype,
    ...
    def _resolve_sender_username(
        self,
        sender_id,
        chat: str = None,
        content: str = None,
    ) -> str:
        """返回某条消息真正发送者的 username；返回空串表示“自己”。

        微信 4.x 的 real_sender_id 数字并不可靠（不是任何一张 name2id 表的
        rowid，实测会出现：文件传输助手/单聊里的自己消息 id 各不相同、
        群聊 id 超过 SenderName2Id 行数、同一 id 在不同会话串成不同好友），
        因此按会话类型分三路解析，保证永不把无关联系人当成发送者：

        1. 群聊（xxx@chatroom）：正文自带 ``wxid_xxx:\\n`` 前缀，直接取前缀；
        2. 单聊（含文件传输助手）：只有“自己 / 会话对方”两种角色——
           先数据驱动识别本账号的“自己 id”（文件传输助手全部是自己，
           见 ``_detect_self_sender_id``），其余一律是会话对方
           （chat username），不再信 SenderName2Id；
        3. 老版本兜底：仍尝试 SenderName2Id 映射（部分旧库 rowid 恰好有效）。
        """
        chat = str(chat or "")
        if content is None:
            content = ""
        is_chatroom = chat.endswith("@chatroom") or chat.endswith("@im.chatroom")
        self_wxid = getattr(self, "wxid", "") or ""

        # ---- 自己：数据驱动识别（不写死 2，微信 4.x 实测为 3） ----
        self_sender_id = self._detect_self_sender_id()
        try:
            sender_id_i = int(sender_id)
        except (TypeError, ValueError):
            sender_id_i = None
        if sender_id_i is not None and sender_id_i == self_sender_id:
            return ""

        # ---- 群聊：正文前缀 wxid 最权威 ----
        if is_chatroom and isinstance(content, str):
            m = _MSG_SENDER_PREFIX_RE.match(content)
            if m:
                wxid = m.group(1)
                return "" if (self_wxid and wxid == self_wxid) else wxid
            # 群聊正文无前缀（自己发的文本/媒体占位）：id 是自己 → 自己
            if sender_id_i is not None and sender_id_i == self_sender_id:
                return ""

        # ---- 单聊：不存在第三方，非自己即会话对方 ----
        if chat and not is_chatroom:
            if chat == "filehelper":
                # 文件传输助手本质是“发给自己的便签”，微信却常把
                # 这些消息记成其它 sender id，一律按自己处理。
                return ""
            if self_wxid and chat == self_wxid:
                return ""  # 与自己的另一个会话（类似文件传输助手）
            return chat  # 会话对方（宁可显示会话名，也不张冠李戴）

        # ---- 兜底：老版本 SenderName2Id 映射 ----
        try:
            if sender_id:
                sender_index = self._sender_id_index()
                su = sender_index.get(int(sender_id), "") or ""
                if su == self_wxid:
                    return ""
                return su
        except (TypeError, ValueError):
    ...
    def _detect_self_sender_id(self) -> int:
        """数据驱动识别“自己”对应的 real_sender_id（微信 4.x 并不总是 2）。

        依据：
        1. 文件传输助手只有自己发言，其文本消息的 real_sender_id 就是自己；
        2. 若文件传输助手为空，统计“出现会话数量最多”的 sender id——
           自己几乎在每个会话都发过言，而好友 id 只出现在各自的会话里；
        3. 以上都拿不到时，退回老版本约定 2。

        结果缓存到实例（切换账号会重建 WeChatDB，缓存自动失效）。
        """
        cached = getattr(self, "_self_sender_id", None)
        if cached is not None:
            return cached
        result: Optional[int] = None
        try:
            # 1) 文件传输助手（最可靠的“只有自己”的会话）
            found = self._msg_conn("filehelper")
            if found:
                conn, table = found
                try:
                    row = conn.execute(
                        "SELECT real_sender_id FROM %s "
                        "WHERE real_sender_id IS NOT NULL AND local_type IN (1) "
                        "GROUP BY real_sender_id ORDER BY COUNT(*) DESC LIMIT 1"
                        % table
                    ).fetchone()
                    if row:
                        result = int(row[0])
                    ...
                    conn.close()
            # 2) 出现会话数最多的 sender id
            if result is None:
                counter: Dict[int, int] = {}
                for c in self.list_message_chats()[:200]:
                    user = c.get("username") or c.get("md5") or ""
                    if not user:
                        continue
                    found = self._msg_conn(user)
                    if not found:
                    ...
                    try:
                        rows = conn.execute(
                            "SELECT DISTINCT real_sender_id FROM %s "
                            "WHERE real_sender_id IS NOT NULL" % table
                        ).fetchall()
                        for (sid,) in rows:
                            try:
            ...
            result = None
        self._self_sender_id = result if result is not None else 2
    ...
    def _sender_id_index(self) -> Dict[int, str]:
        """(仅作兜底) real_sender_id → 用户名，来自 message_resource.SenderName2Id。

        注意：微信 4.x 部分版本/账号下 real_sender_id 并不是该表 rowid，
        此时该映射会张冠李戴，正式解析请走 ``_resolve_sender_username``。
        """
    ...
    def _resolve_sender(
        ...
        self_sender_id = self._detect_self_sender_id()
        try:
            if int(sender_id) == self_sender_id:
                return self_nick
        except (TypeError, ValueError):
            pass
        if sender_id in (2, "2") and self_sender_id == 2:
            return self_nick
        chat = str(chat or "")
        is_chatroom = chat.endswith("@chatroom") or chat.endswith("@im.chatroom")
        if is_chatroom and isinstance(content, str):
            ...
            if m:
                ...
                if wxid == self.wxid:
                    return self_nick
                return nicks.get(wxid, wxid) or wxid
        if chat and not is_chatroom and chat != self.wxid:
            if chat == "filehelper":
                return self_nick
            return nicks.get(chat, chat) or chat
    ...
    def export_history(
        ...
        try:
            ...
            for i, md5 in enumerate(order):
                ...
                msgs = []
                for r in rows:
                    out = self._export_row(r, MSG_TYPE_NAMES)
                    out["sender_name"] = self._resolve_sender(
                        r["real_sender_id"], sender_index, nicks,
                        self_info.get("nick_name", "我"),
                        chat=user, content=out.get("content"),
                    )
                    msgs.append(out)
                total += len(msgs)
                chat_info.append({
