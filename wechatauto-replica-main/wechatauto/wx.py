...
def _db_row_to_message(row: dict, chat: 'Chat', self_wxid: str = None) -> 'Message':
    ...
    也支持 db.py 解析出的 ``sender_username == self_wxid`` 与文件传输助手
    特例（微信 4.x 常把这类消息的 sender 记成其它 id）。
    """
    ...
    chat_wxid = str(getattr(chat, '_wxid', '') or getattr(chat, 'who', '') or '')
    # 自己判定：官方约定 id=2；或解析出的 username 就是本账号；
    # 或文件传输助手（本质是发给自己的便签，微信 4.x 常把 sender 记成其它 id）。
    is_self = (
        sender_id in (2, "2")
        or bool(self_wxid and sender_username == str(self_wxid))
        or chat_wxid == 'filehelper'
        or (sender_username == '' and sender_id not in (None, ''))
    )
