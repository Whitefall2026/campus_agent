...
- **微信自动提取**：读取并监听微信 4.x 聊天消息（wechatauto-replica），把含
  ...
  全部会话
- **公众号推送转待办**：自动发现并实时读取已关注公众号的文本与图文推送，解析
  多图文标题、摘要和原文链接；仅把含报名、提交、缴费、预约、截止、活动通知等
  行动信号的内容同步到「待采纳」，用户确认后才写入待办
...
Windows 安装版无需目标电脑安装 Python。双击安装后，从开始菜单启动
「RUC Agent 校园管家」；应用会启动本地服务并自动打开浏览器，控制窗口可再次
打开页面、打开数据目录或安全退出。

安装版的个人数据保存在 `%LOCALAPPDATA%\RUC Agent\data`，与程序文件分离，
升级或卸载程序不会覆盖日程、画像和 API Key。微信自动提取仍要求本机安装并
登录微信 4.1.12+。
...
最终安装程序位于 `dist\installer\RUC-Agent-Setup-1.0.0.exe`。构建脚本会先运行
测试，再对冻结后的 exe 做静态页面与核心 API 冒烟验证。
...
勾选“自动读取公众号推送并识别待办”后，程序会自动发现微信数据库中用户名以
`gh_` 开头的公众号会话，无需逐个填写公众号名称；运行期间每约 20 秒发现新出现
的公众号会话。普通资讯和回顾类文章不会写入待办，命中的推送会显示在「对话」页
待采纳区，并保留“查看公众号原文”入口。忽略或采纳后不会因重连、扫描再次弹出。
